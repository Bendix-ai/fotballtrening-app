// English translations for the app
import { Translations } from './no';

export const en: Translations = {
    // Common
    common: {
        loading: 'Loading...',
        error: 'Oops! Something went wrong',
        retry: 'Try again',
        errorDesc: "Don't worry, just try again!",
        cancel: 'Cancel',
        save: 'Save',
        delete: 'Delete',
        edit: 'Edit',
        back: 'Back',
        next: 'Next',
        done: 'Done',
        yes: 'Yes',
        no: 'No',
        ok: 'OK',
        add: 'Add',
        search: 'Search',
        close: 'Close',
        confirm: 'Confirm',
        select: 'Select...',
    },

    // Onboarding
    onboarding: {
        welcome: 'Welcome to FotballTrening!',
        subtitle: 'Train smarter, compete with teammates',
        welcomeDesc: 'Train smarter, compete with teammates, and become a better football player.',
        howItWorks: 'How it works',
        howItWorksDesc: 'Choose an exercise, complete it, and earn points! Exercises are adapted to your level.',
        competeTitle: 'Compete and win',
        competeDesc: 'Climb the leaderboard, unlock achievements, and keep your streak going!',
        selectClub: 'Select your club',
        selectTeam: 'Select your team',
        getStarted: 'Get started',
        skip: 'Skip',
    },

    // Auth
    auth: {
        login: 'Log in',
        logout: 'Log out',
        username: 'Username',
        password: 'Password',
        forgotPassword: 'Forgot password?',
        loginButton: 'Log in',
        loginError: 'Hmm, that username or password was wrong. Try again!',
        welcomeBack: 'Welcome back!',
        selectClub: 'Select club',
        selectYear: 'Select year group',
        selectGender: 'Select gender',
        createAccount: 'Create account',
        createAccountButton: 'Create account',
        displayName: 'Display name',
        confirmPassword: 'Confirm password',
        passwordMismatch: 'Passwords do not match',
        alreadyHaveAccount: 'Already have an account?',
        noAccount: "Don't have an account?",
        registrationSuccess: 'Account created!',
        registrationFailed: 'Oops! Registration failed. Try again!',
        clubNotFound: "Can't find your club?",
        createNewClub: 'Create new club',
        clubName: 'Club name',
        backToClubList: 'Back to club list',
        selectTeams: 'Select teams you coach',
        teamAdmin: 'Coach',
        clubAdmin: 'Club admin',
        player: 'Player',
        atLeastOneTeam: 'Select at least one team',
        forgotPasswordTitle: 'Reset password',
        forgotPasswordDesc: "Enter your registered email and we'll send you a reset link.",
        sendResetLink: 'Send reset link',
        resetEmailSent: 'Email sent!',
        resetEmailSentDesc: 'Check your inbox for a password reset link.',
        backToLogin: 'Back to login',
        resetPassword: 'Set new password',
        resetPasswordDesc: 'Enter your new password below.',
        passwordUpdated: 'Password updated!',
        playerForgotPassword: 'Contact your coach to get a new password',
        changePassword: 'Change password',
        currentPassword: 'Current password',
        newPassword: 'New password',
        confirmNewPassword: 'Confirm new password',
        clubNameMinLength: 'Club name must be at least 2 characters',
        fillEmail: 'Enter email',
        fillUsername: 'Enter username',
        fillDisplayName: 'Enter display name',
        fillPassword: 'Enter password',
        accountCreatedLogin: 'Account created! Log in to continue.',
        clubNamePlaceholder: 'Enter club name...',
        selectClubPlaceholder: 'Select club...',
        selectYearPlaceholder: 'Select year group...',
        selectGenderPlaceholder: 'Select gender...',
        noTeamsFound: 'No teams found for this club',
        displayNamePlaceholder: 'Display name',
        emailLabel: 'Email',
        enterCurrentPassword: 'Enter current password',
        passwordMinLength: 'New password must be at least 6 characters',
        enterPasswordAgain: 'Enter password again',
        minChars: 'At least 6 characters',
        adminResetPassword: 'Reset password',
        adminResetPasswordConfirm: "Are you sure you want to reset this player's password?",
        adminResetPasswordSuccess: 'Password has been reset',
        fillCredentials: 'Please enter username and password',
        profileLoadError: 'Could not load profile. Please try again.',
        loginFailed: 'Login failed',
        wrongCredentials: 'Wrong username or password',
        emailNotConfirmed: 'Your email is not confirmed yet. Contact your coach.',
        email: 'Email',
        changeTeam: 'Change team',
        savedAs: 'Saved as',
    },

    // Navigation tabs
    tabs: {
        home: 'Home',
        exercises: 'Exercises',
        leaderboard: 'Leaderboard',
        profile: 'Profile',
    },

    // Home screen
    home: {
        greeting: 'Hi',
        todayProgress: "Today's progress",
        exercises: 'exercises',
        points: 'points',
        currentStreak: 'Current streak',
        days: 'days',
        quickStart: 'Quick start',
        viewAll: 'View all',
        keepGoing: 'Keep it up!',
        startTraining: 'Start training',
        todayChallenge: "Today's challenge",
        recentExercises: 'Recent exercises',
        dailyGoal: 'Daily goal',
        dailyGoalProgress: '{current} of {target} exercises',
        dailyGoalComplete: 'Daily goal reached!',
        yourRank: "You're #{rank} in the club",
        level: 'Level {level}',
        pointsToNextLevel: '{points} points to next level',
        maxLevel: 'Max level!',
        levelUp: 'Level Up!',
        nowLevel: 'You are now level {level}!',
        announcements: 'Announcements',
        teamActivity: 'Team activity',
        completedExercise: '{name} completed {exercise}',
        unlockedAchievement: '{name} unlocked {achievement}',
        reachedStreak: '{name} reached {days} day streak!',
        todayTraining: "Today's training",
        startDailyPlan: 'Start daily plan',
        seeMore: 'See more',
        exercisesAvailable: '{count} exercises available',
        welcomeNewUser: "Welcome! Let's get started",
        firstExercise: 'Start your first exercise',
        newUserTip: 'Complete an exercise to earn your first points!',
        activeChallenges: 'Active challenges',
        activeChallengesCount: '{count} active challenges',
        highFive: 'Give high five!',
        highFiveSent: 'High five sent!',
        friendActivity: 'Friend activity',
        loginBonus: 'Login bonus!',
        loginBonusPoints: '+{points} points',
        dailyChallengeBonus: 'Daily challenge — 2x points!',
        doublePoints: '2x points',
        hoursRemaining: '{hours}h {minutes}m remaining',
    },

    // Exercises
    exercises: {
        title: 'Exercises',
        all: 'All',
        warmup: 'Warm-up',
        strength: 'Strength',
        agility: 'Agility',
        skill: 'Skill',
        cooldown: 'Cool-down',
        difficulty: 'Difficulty',
        easy: 'Easy',
        medium: 'Medium',
        hard: 'Hard',
        duration: 'Duration',
        seconds: 'seconds',
        minutes: 'minutes',
        points: 'points',
        start: 'Start exercise',
        complete: 'Complete',
        instructions: 'Instructions',
        completed: 'Completed!',
        pointsEarned: 'You earned {points} points!',
        greatJob: 'Great job!',
        continueTraining: 'Continue training',
        backToExercises: 'Back to exercises',
        search: 'Search exercises...',
        favorites: 'Favorites',
        paused: 'Paused',
        remaining: 'Remaining',
        stepOf: 'Step {current} of {total}',
        resume: 'Resume',
        pause: 'Pause',
        exitTitle: 'Exit exercise?',
        exitMessage: 'Are you sure you want to exit? Your progress will be lost.',
        exit: 'Exit',
        halfway: 'Halfway there!',
        almostThere: 'Almost there!',
        watchVideo: 'Watch video',
        notFound: 'Exercise not found',
        equipment: 'Equipment',
        relatedExercises: 'Related exercises',
        addToFavorites: 'Add to favorites',
        removeFromFavorites: 'Remove from favorites',
        doAnother: 'Do another exercise',
        seeProgress: 'See progress',
        backToHome: 'Back to home',
        levelProgress: 'Level progress',
    },

    // Leaderboard
    leaderboard: {
        title: 'Leaderboard',
        club: 'Club',
        yearGroup: 'Year group',
        team: 'Team',
        friends: 'Friends',
        thisWeek: 'This week',
        thisMonth: 'This month',
        allTime: 'All time',
        rank: 'Rank',
        player: 'Player',
        points: 'Points',
        you: 'You',
        noData: 'No data yet',
        rankUp: 'up',
        rankDown: 'down',
        climbedPositions: 'You climbed {count} positions this week!',
    },

    // Profile
    profile: {
        title: 'My profile',
        totalPoints: 'Total points',
        exercisesCompleted: 'Exercises completed',
        currentStreak: 'Current streak',
        longestStreak: 'Longest streak',
        achievements: 'Achievements',
        viewAll: 'View all',
        settings: 'Settings',
        days: 'days',
        activityHistory: 'Activity history',
        changePhoto: 'Change profile picture',
        photoUpdated: 'Profile picture updated!',
        photoError: 'Could not update profile picture',
        friends: 'Friends',
        addFriend: 'Add friend',
        removeFriend: 'Remove friend',
        noFriends: 'No friends yet',
        friendAdded: 'Friend added!',
        searchFriends: 'Search for username...',
        friendRemoved: 'Friend removed',
        alreadyFriends: 'Already friends',
        userNotFound: 'User not found',
    },

    // Achievements
    achievements: {
        title: 'Achievements',
        locked: 'Locked',
        unlocked: 'Unlocked',
        first_exercise: 'First exercise',
        first_exercise_desc: 'Complete your first exercise',
        streak_7: 'Week streak',
        streak_7_desc: 'Keep your streak for 7 days',
        streak_30: 'Month streak',
        streak_30_desc: 'Keep your streak for 30 days',
        points_100: 'Hundred points',
        points_100_desc: 'Earn 100 points',
        points_500: 'Five hundred',
        points_500_desc: 'Earn 500 points',
        points_1000: 'Thousand points',
        points_1000_desc: 'Earn 1000 points',
        exercises_10: 'Ten exercises',
        exercises_10_desc: 'Complete 10 exercises',
        exercises_50: 'Fifty exercises',
        exercises_50_desc: 'Complete 50 exercises',
        streak_3: 'Three day streak',
        streak_3_desc: 'Keep your streak for 3 days',
        points_10: 'Ten points',
        points_10_desc: 'Earn 10 points',
        exercises_5: 'Five exercises',
        exercises_5_desc: 'Complete 5 exercises',
        first_favorite: 'First favorite',
        first_favorite_desc: 'Add your first favorite exercise',
        tried_2_categories: 'Tried two categories',
        tried_2_categories_desc: 'Complete exercises in at least 2 categories',
        first_challenge: 'First challenge',
        first_challenge_desc: 'Send your first challenge',
        first_highfive: 'First high five',
        first_highfive_desc: 'Give your first high five',
        all_categories: 'All-rounder',
        all_categories_desc: 'Try all exercise categories',
        nextAchievement: 'Next achievement',
        progressLabel: '{current}/{target}',
    },

    // Settings
    settings: {
        title: 'Settings',
        appearance: 'Appearance',
        theme: 'Theme',
        light: 'Light',
        dark: 'Dark',
        system: 'System',
        notifications: 'Notifications',
        enableNotifications: 'Enable notifications',
        about: 'About',
        version: 'Version',
        logout: 'Log out',
        logoutConfirm: 'Are you sure you want to log out?',
        changePassword: 'Change password',
        language: 'Language',
        general: 'General',
        sounds: 'Sounds',
        soundsDescription: 'Play sounds on events',
    },

    // Notifications
    notifications: {
        dailyReminder: 'Daily reminder',
        dailyReminderDesc: 'Get a daily reminder to train',
        newExercises: 'New exercises',
        newExercisesDesc: 'Get notified when new exercises are available',
        leaderboard: 'Leaderboard updates',
        leaderboardDesc: 'Get notified about leaderboard changes',
        streakReminder: 'Streak reminder',
        streakReminderDesc: 'Get a reminder before your streak breaks',
        note: 'Notifications may take a few minutes to activate. You can change these settings at any time.',
        pushComingSoon: 'Push notifications are coming in a future update. Your settings are saved and will apply when the feature is ready.',
        permissionDenied: 'Notification permission was denied. Enable notifications in settings.',
        dailyReminderTitle: 'Time to train!',
        dailyReminderBody: 'Open the app and train today 💪',
        streakReminderTitle: 'Your streak is at risk!',
        streakReminderBody: 'Complete an exercise before midnight 🔥',
    },

    // About
    about: {
        title: 'About',
        version: 'Version',
        description: 'FotballTrening helps young football players train smarter, track progress, and compete with teammates. Complete exercises, earn points, and climb the leaderboard!',
        developer: 'Developer',
        privacy: 'Privacy policy',
        copyright: '© 2024 FotballTrening AS. All rights reserved.',
    },

    // Admin
    admin: {
        // Navigation
        dashboard: 'Dashboard',
        players: 'Players',
        exercises: 'Exercises',
        clubStructure: 'Club structure',
        exerciseStore: 'Exercise store',
        reports: 'Reports',
        adminSettings: 'Settings',

        // Dashboard
        totalPlayers: 'Total players',
        activeLast7Days: 'Active last 7 days',
        totalCompletions: 'Total completions',
        engagementRate: 'Engagement',
        recentActivity: 'Recent activity',
        topPerformers: 'Top performers',
        quickActions: 'Quick actions',
        alerts: 'Alerts',
        activeToday: 'Active today',
        exercisesToday: 'Exercises today',

        // Players
        addPlayer: 'Add player',
        editPlayer: 'Edit player',
        deletePlayer: 'Delete player',
        deletePlayerConfirm: 'Are you sure you want to delete this player?',
        playerName: 'Player name',
        username: 'Username',
        password: 'Password',
        confirmPassword: 'Confirm password',
        yearGroup: 'Year group',
        gender: 'Gender',
        allYears: 'All year groups',
        allGenders: 'All genders',
        boys: 'Boys',
        girls: 'Girls',
        mixed: 'Mixed',
        lastActive: 'Last active',
        active: 'Active',
        inactive: 'Inactive',
        noPlayersFound: 'No players found',

        // Club Structure
        clubInfo: 'Club information',
        yearGroups: 'Year groups',
        addYearGroup: 'Add year group',
        boysCount: 'Boys',
        girlsCount: 'Girls',
        totalCount: 'Total',

        // Exercises
        addExercise: 'Add exercise',
        editExercise: 'Edit exercise',
        deleteExercise: 'Delete exercise',
        clubExercises: 'Club exercises',
        customExercises: 'Custom exercises',
        title: 'Title',
        description: 'Description',
        category: 'Category',
        difficulty: 'Difficulty',
        duration: 'Duration',
        points: 'Points',
        instructions: 'Instructions',
        visibility: 'Visibility',
        public: 'Public',
        private: 'Private',
        assignToTeams: 'Assign to teams',
        allTeams: 'All teams',
        assignedTeams: '{count} teams selected',

        // Exercise Store
        featured: 'Featured',
        popular: 'Popular',
        newest: 'Newest',
        allExercises: 'All exercises',
        addToClub: 'Add to club',
        addedToClub: 'Added to club!',
        rating: 'Rating',
        downloads: 'Downloads',
        reviews: 'Reviews',
        by: 'By',
        importStarterPack: 'Import starter pack',
        starterPackImported: 'Starter pack imported!',
        starterPackDesc: 'Download featured exercises to your club',
        searchStore: 'Search the store...',
        results: 'results',

        // Reports
        dateRange: 'Date range',
        last7Days: 'Last 7 days',
        last30Days: 'Last 30 days',
        last90Days: 'Last 90 days',
        weeklyActivity: 'Weekly activity',
        monthlyPoints: 'Monthly points',
        categoryDistribution: 'Category distribution',
        difficultyDistribution: 'Difficulty distribution',
        exportPDF: 'Export PDF',
        exportCSV: 'Export CSV',
        exportSuccess: 'Export started!',
        noReportData: 'No data yet',
        noReportDataDesc: 'Reports will appear here once players start completing exercises.',
        daySun: 'Sun',
        dayMon: 'Mon',
        dayTue: 'Tue',
        dayWed: 'Wed',
        dayThu: 'Thu',
        dayFri: 'Fri',
        daySat: 'Sat',
        monthJan: 'Jan',
        monthFeb: 'Feb',
        monthMar: 'Mar',
        monthApr: 'Apr',
        monthMay: 'May',
        monthJun: 'Jun',
        monthJul: 'Jul',
        monthAug: 'Aug',
        monthSep: 'Sep',
        monthOct: 'Oct',
        monthNov: 'Nov',
        monthDec: 'Dec',
        catWarmup: 'Warm-up',
        catStrength: 'Strength',
        catAgility: 'Agility',
        catSkill: 'Skill',
        catCooldown: 'Cool-down',
        catOther: 'Other',
        diffEasy: 'Easy',
        diffMedium: 'Medium',
        diffHard: 'Hard',
        unknownPlayer: 'Unknown',
        completedAction: 'Completed {exercise}',
        unknownExercise: 'Unknown exercise',

        // Admin Settings
        profileSection: 'Profile',
        clubSettings: 'Club settings',
        clubName: 'Club name',
        support: 'Support',
        contactSupport: 'Contact support',
        helpCenter: 'Help center',
        clubAdministrator: 'Club administrator',
        teamCoach: 'Coach',
        managedTeams: 'Your teams',
        administrator: 'Administrator',
        emailNotifications: 'Email notifications',
        pushNotifications: 'Push notifications',
        setupChecklist: 'Get started',
        setupChecklistDesc: 'Complete these steps to set up your club',
        setupYearGroups: 'Add year groups',
        setupPlayers: 'Add players',
        setupExercises: 'Add exercises',

        // Player Detail
        playerDetail: 'Player details',
        recentCompletions: 'Recent completions',
        noCompletions: 'No completions yet',
        memberSince: 'Member since',

        // Announcements
        announcements: 'Announcements',
        newAnnouncement: 'New announcement',
        announcementTitle: 'Title',
        announcementMessage: 'Message',
        noAnnouncements: 'No announcements yet',
        sendAnnouncement: 'Send announcement',
        announcementSent: 'Announcement sent!',

        // Training Plans
        trainingPlans: 'Training plans',
        weeklyPlan: 'Weekly plan',
        monday: 'Monday',
        tuesday: 'Tuesday',
        wednesday: 'Wednesday',
        thursday: 'Thursday',
        friday: 'Friday',
        addExerciseToDay: 'Add exercise',
        noPlannedExercises: 'No planned exercises today',
        todaysPlan: "Today's plan",
        planSaved: 'Plan saved!',
        selectExercise: 'Select exercise',
        removeExercise: 'Remove exercise',
        thisWeeksPlan: "This week's plan",
        // AddEditPlayer form
        fillPlayerName: 'Please enter player name',
        fillUsername: 'Please enter username',
        fillPassword: 'Please enter password',
        selectYearGroup: 'Please select year group',
        selectGender: 'Please select gender',
        playerUpdated: 'Player updated',
        playerCreated: 'Player created',
        usernameInUse: 'Username is already in use',
        fullNamePlaceholder: 'Full name',
        usernamePlaceholder: 'username',
        keepCurrentPassword: 'Leave empty to keep current',
        selectYearPlaceholder: 'Select year group...',
        selectGenderPlaceholder: 'Select gender...',
        fillYear: 'Please enter a year',
        invalidYear: 'Invalid year (1990–2030)',
        selectAtLeastOneGender: 'Select at least one gender',
        yearGroupAdded: 'Year group added!',
        yearGroupAddError: 'Could not add year group',
    },

    // Help Center
    helpCenter: {
        title: 'Help center',
        gettingStarted: 'Getting started',
        gettingStartedDesc: 'Follow these steps to set up your club and get started with FotballTrening.',
        quickSteps: 'Quick start',
        step1: 'Add year groups under Club Structure',
        step2: 'Create players with username and password',
        step3: 'Add exercises or download from the store',
        step4: 'Players log in and start training!',
        faq: 'Frequently asked questions',
        faqAddPlayers: 'How do I add players?',
        faqAddPlayersAnswer: 'Go to Players in the menu, tap the + button, and fill in name, username, and password. Select year group and gender.',
        faqAddExercises: 'How do I create exercises?',
        faqAddExercisesAnswer: 'Go to Exercises in the menu, tap the + button. Fill in title, description, category, difficulty, and instructions.',
        faqStore: 'What is the exercise store?',
        faqStoreAnswer: 'The exercise store contains ready-made exercises you can download to your club. Tap Download to add them.',
        faqReports: 'How do I view reports?',
        faqReportsAnswer: 'Go to Reports in the menu. You can see activity, points, and distribution. Use the export buttons for PDF or CSV.',
        faqPlayerLogin: 'How do players log in?',
        faqPlayerLoginAnswer: 'Players select club, year group, and gender on the login page, then enter the username and password you created.',
        faqResetPassword: 'How do I reset a password?',
        faqResetPasswordAnswer: 'Go to Players, tap the player, and select Reset Password. A new password is sent or can be set manually.',
    },

    // Streak
    streak: {
        title: 'Streak',
        keepItUp: 'Keep it up!',
        broken: 'Streak broken',
        newRecord: 'New record!',
        shieldAvailable: 'Streak shield available',
        useShield: 'Use streak shield?',
        shieldUsed: 'Streak shield used!',
        dontGiveUp: "Don't give up! Start a new streak",
        startNew: 'Start new streak',
    },

    // Mascot
    mascot: {
        ready: 'Ready for your first exercise? Let\'s go!',
        impressiveStreak: 'Impressive streak! {days} days in a row!',
        greatJob: 'Great job! You\'re on your way!',
        keepGoing: 'Fantastic effort! Keep it up!',
        howAboutTraining: 'How about a little training today?',
        streakInDanger: 'Your streak is at risk! Can you fit in an exercise?',
        goodMorning: 'Good morning! Ready for training?',
        goodAfternoon: 'Good afternoon! Time for some training?',
        goodEvening: 'Good evening! Can you fit in an exercise?',
        lateNight: 'Up late? A quick exercise maybe?',
        champion: 'You\'re a champion! Keep up the great work!',
        almostThere: 'Almost there! One more exercise?',
        newDay: 'New day, new opportunities!',
        proud: 'Proud of you! Great effort!',
        letsTrain: 'Let\'s train! What do you choose today?',
        superStar: 'You\'re a superstar!',
        neverGiveUp: 'Never give up! You can do it!',
        teamPlayer: 'Great teamwork starts with training!',
        warmUp: 'Remember to warm up before you start!',
        coolDown: 'Great session! Remember to cool down!',
    },

    // Challenges
    challenges: {
        title: 'Challenges',
        challengeFriend: 'Challenge a friend',
        createChallenge: 'Send challenge',
        selectOpponent: 'Select opponent',
        bonusPoints: 'Bonus points',
        pending: 'Pending',
        accepted: 'Accepted',
        completed: 'Completed',
        expired: 'Expired',
        noChallenges: 'No challenges yet',
        noChallengesDesc: 'Challenge a teammate to complete an exercise!',
        challengeSent: 'Challenge sent!',
        challengeAccepted: 'Challenge accepted!',
        youWon: 'You won!',
        youLost: 'You lost',
        draw: 'Draw!',
        accept: 'Accept',
        decline: 'Decline',
        vs: 'vs',
        active: 'Active',
        history: 'History',
        expiresIn: 'Expires in {hours}h',
        winnerBonus: '+{points} bonus points to the winner!',
        friends: 'Friends',
        teammates: 'Teammates',
    },
};

export default en;
